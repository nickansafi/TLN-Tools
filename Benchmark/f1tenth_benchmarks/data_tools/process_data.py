...
def process_data():
    ...
    folders = glob.glob(os.path.abspath(__file__).rpartition("/Benchmark")[0]+"/Benchmark/f1tenth_benchmarks/benchmark_results/Logs/*")
    ...
    full_df.to_csv(os.path.abspath(__file__).rpartition("/Benchmark")[0]+"/Benchmark/f1tenth_benchmarks/benchmark_results/Logs/Full.csv", index=False, float_format='%.4f')
    summary_df = pd.DataFrame(summary_df)
    summary_df['VehicleID'] = summary_df['Vehicle'] + "_" + summary_df['TestID']
    summary_df = summary_df.sort_values(by=["Vehicle", "MapName"])
    summary_df.to_csv(os.path.abspath(__file__).rpartition("/Benchmark")[0]+"/Benchmark/f1tenth_benchmarks/benchmark_results/Logs/Summary.csv", index=False, float_format='%.4f')
